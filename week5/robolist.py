def count_steps(numbers):
    sorted_with_index_list = []
    for i, num in enumerate(numbers):
        sorted_with_index_list.append((num, i))

    sorted_with_index = sorted(sorted_with_index_list)
    
    total_steps = 0
    current_position = 0
    
    for _, index in sorted_with_index:
        steps = abs(index - current_position)
        total_steps += steps
